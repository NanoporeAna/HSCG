...
class GINConv(MessagePassing):
    def __init__(self, emb_dim, aggr="add", smiles_version='version1'):
...
class GNN(nn.Module):
    def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0., gnn_type="gin", smiles_version='version1'):
        if num_layer < 2:
        ...
        for layer in range(num_layer):
            if gnn_type == "gin":
                self.gnns.append(GINConv(emb_dim, aggr="add", smiles_version=smiles_version))
            elif gnn_type == "gcn":
...
class GNN_graphpred(nn.Module):
    def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0, graph_pooling="mean", gnn_type="gin", feat_chang_dim=512, feat_dim=768, smiles_version='version1'):
        ...
        self.feat_chang_dim = feat_chang_dim
        ...
        self.gnn = GNN(num_layer, emb_dim, JK, drop_ratio, gnn_type=gnn_type, smiles_version=smiles_version)
        ...
        if self.JK == "concat":
            self.graph_pred_linear = torch.nn.Linear(self.mult * (self.num_layer + 1) * self.emb_dim, self.emb_dim)
        else:
            self.graph_pred_linear = torch.nn.Linear(self.mult * self.emb_dim, self.emb_dim)

        #模型输出约束变换
        self.feat_lin = torch.nn.Linear(self.emb_dim, self.feat_chang_dim)

        self.out_lin = torch.nn.Sequential(
            torch.nn.Linear(self.feat_chang_dim, self.feat_chang_dim),
            torch.nn.ReLU(inplace=True),
            # nn.Softplus(),
            torch.nn.Linear(self.feat_chang_dim, self.feat_dim)
        )
