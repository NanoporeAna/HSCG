...
class Load_Data:
    ...
    def load_data_old(self, files, data_type):
        ...
        if isinstance(files, list):
            ...
            all_ids = data[self.ID_type].values
        ...
        if data_type == 'eval':
            ...
            if self.have_tags:
                df['tags'] = all_tags[index::]
            print( df['id'][0])
            return data,df, all_smiles, all_ids, all_wts
    ...
    def load_data_new(self, files, data_type):
        ...
        if data_type == 'eval':
            if len(all_hsqcs) != len(all_smiless): 
                ...
                val_wts = all_wts[int(len(all_hsqcs)*self.data_reat):]
                val_data = datas[int(len(all_hsqcs)*self.data_reat):]
                ...
                if len(val_hsqc) != len(val_data):
                    print("data have error !")
                    sys.exit(1)
            return datas, val_data, all_smiless, val_hsqc, val_tags, val_wts
